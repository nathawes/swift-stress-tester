...
public typealias SKResult<T> = Result<T, ResponseError>
...
extension ResponseError {
  ...
  public static func custom(_ message: String) -> ResponseError {
    return ResponseError(code: .custom, message: message)
  }
}


/// A wrapper for accessing the API of a sourcekitd library loaded via `dlopen`.
public final class SwiftSourceKitFramework {
  ...
  public init(dylib path: AbsolutePath = AbsolutePath("/Users/nhawes/github/build/toolchain/swift-macosx-x86_64/lib/sourcekitd.framework/sourcekitd")) throws {
    self.path = path
...
extension SwiftSourceKitFramework {
  ...
  /// Send the given request and synchronously receive a reply dictionary (or error).
  public func send(_ req: SKRequestDictionary, reply: @escaping (SKResult<SKResponseDictionary>) -> ()) -> sourcekitd_request_handle_t? {
    logAsync { _ in req.description }
...
public struct sourcekitd_keys {
  ...
  public let enablesubstructure: sourcekitd_uid_t
  public let syntaxtreetransfermode: sourcekitd_uid_t
  public let syntax_tree_serialization_format: sourcekitd_uid_t
  public let syntactic_only: sourcekitd_uid_t
  public let serialized_syntax_tree: sourcekitd_uid_t
  ...
  init(api: sourcekitd_functions_t) {
    ...
    enablesubstructure = api.uid_get_from_cstr("key.enablesubstructure")!
    syntaxtreetransfermode = api.uid_get_from_cstr("key.syntaxtreetransfermode")!
    syntax_tree_serialization_format = api.uid_get_from_cstr("key.syntax_tree_serialization_format")!
    syntactic_only = api.uid_get_from_cstr("key.syntactic_only")!
    serialized_syntax_tree = api.uid_get_from_cstr("key.serialized_syntax_tree")!
...
public struct sourcekitd_requests {
  ...
  public let refactoring: sourcekitd_uid_t
  ...
  init(api: sourcekitd_functions_t) {
    ...
    refactoring = api.uid_get_from_cstr("source.request.semantic.refactoring")!
  }
}
...
public struct sourcekitd_values {
  ...
  // syntax tree transfer modes
  public let syntaxtree_transfer_full: sourcekitd_uid_t
  public let syntaxtree_transfer_incremental: sourcekitd_uid_t
  public let syntaxtree_transfer_off: sourcekitd_uid_t

  // syntax tree serialization formats
  public let syntaxtree_serialization_format_json: sourcekitd_uid_t
  public let syntaxtree_serialization_format_bytetree: sourcekitd_uid_t
  ...
  init(api: sourcekitd_functions_t) {
    ...
    syntaxtree_transfer_full = api.uid_get_from_cstr("source.syntaxtree.transfer.full")!
    syntaxtree_transfer_incremental = api.uid_get_from_cstr("source.syntaxtree.transfer.incremental")!
    syntaxtree_transfer_off = api.uid_get_from_cstr("source.syntaxtree.transfer.off")!
    syntaxtree_serialization_format_json = api.uid_get_from_cstr("source.syntaxtree.serialization.format.json")!
    syntaxtree_serialization_format_bytetree = api.uid_get_from_cstr("source.syntaxtree.serialization.format.bytetree")!
  }
}
...
public final class SKRequestDictionary {
  ...
  public init(_ dict: sourcekitd_object_t? = nil, sourcekitd: SwiftSourceKitFramework) {
  ...
  public subscript(key: sourcekitd_uid_t?) -> SKRequestArray {
    get { fatalError("request is set-only") }
    set { sourcekitd.api.request_dictionary_set_value(dict, key, newValue.array) }
  }
}

public final class SKRequestArray {
  let array: sourcekitd_object_t?
  let sourcekitd: SwiftSourceKitFramework

  public init(_ array: sourcekitd_object_t? = nil, sourcekitd: SwiftSourceKitFramework) {
...
public final class SKResponse {
  ...
  public var value: SKResponseDictionary? {
    if sourcekitd.api.response_is_error(response) {
      return nil
    }
...
public final class SKResponseDictionary {
  ...
  public subscript(key: sourcekitd_uid_t?) -> Data? {
    let data = sourcekitd.api.variant_dictionary_get_value(dict, key)
    guard let ptr = sourcekitd.api.variant_data_get_ptr(data) else { return nil }
    let size = sourcekitd.api.variant_data_get_size(data)
...
public final class SKResponseArray {
  ...
  @discardableResult
  public func forEach(_ applier: (Int, SKResponseDictionary) throws -> Bool) rethrows -> Bool {
    for i in 0..<count {
      if try !applier(i, SKResponseDictionary(sourcekitd.api.variant_array_get_value(array, i), response: resp)) {
        return false
      }
    }
    return true
